...
func (m app_model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.MouseMsg:
		if zone.Get("input").InBounds(msg) {
			m.input.Focus()
		}
		if zone.Get("table").InBounds(msg) {
...
func (m app_model) View() string {
	// Init flex container
	view := flexbox.New(m.width, m.height)

	// Update Views
	btn1 := m.button_1.View()
	btn2 := m.button_2.View()

	// Before updating input view... set width
	m.input.Width = m.width

	// Update input view
	inpt := m.input.View()

	// Before updating table view... set styles
	...
	columns := []*flexbox.Row{
		...
		view.NewRow().AddCells(
			flexbox.NewCell(3, 11).
				// SetStyle(lipgloss.NewStyle().Background(lipgloss.Color("#FF0000"))).
				SetContent(zone.Mark("table",
					// Not working...
					// lipgloss.NewStyle().MarginBottom(500).Render(tble),
					tble,
				)),
		),
	}
	view.AddRows(columns)
...
func (m button_model) View() string {
	...
	button := zone.Mark(m.zone_id, btnStyle.Render(btnMsg))
